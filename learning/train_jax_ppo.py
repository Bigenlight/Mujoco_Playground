...
_INTERACTIVE = flags.DEFINE_boolean(
    "interactive", False,
    "If true and --play_only, drop into a real-time mujoco.viewer instead of saving a video"
)
...
def main(argv):
  ...
  if _LOAD_CHECKPOINT_PATH.value is not None:
    ...
    if ckpt_path.is_dir():
      ...
      latest_ckpt = latest_ckpts[-1] # 여기 수정하면 원하는 체크포인트에서 영상 생성 가능 (-1이 마지막)
...
#####
  # If the user asked for interactive play-only, hook up mujoco.viewer:
  if _PLAY_ONLY.value and _INTERACTIVE.value:
    import mujoco.viewer as mv

    # a tiny loader+control callback:
    def control_cb(model: mujoco.MjModel, data: mujoco.MjData):
      # build obs exactly as your training loop did
      obs = np.concatenate([
        data.qpos[:env_cfg.observation_size // 2],
        data.qvel[:env_cfg.observation_size // 2]
      ])[None, :].astype(np.float32)
      rng_key = jax.random.PRNGKey(0)  # or carry one around
      action = np.array(jit_inference_fn(obs, rng_key)[0], np.float32)
      data.ctrl[:] = np.clip(action,
                             model.actuator_ctrlrange[:,0],
                             model.actuator_ctrlrange[:,1])

    def load_cb(model=None, data=None):
      mujoco.set_mjcb_control(control_cb)
      xml = epath.Path(env_cfg.xml_path).read_text()
      m = mujoco.MjModel.from_xml_string(xml, assets=manipulation.get_assets())
      return m, mujoco.MjData(m)

    return mv.launch(loader=load_cb)
######
  ...
  total_sec = eval_end - eval_start
  print(f"Inference (no-render) total: {total_sec:.3f}s, per step: {total_sec/num_steps:.4f}s")
  # ------------------------------------------------------------------------
