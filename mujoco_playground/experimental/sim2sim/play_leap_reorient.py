...
class OnnxController:
  ...
  def __init__(
    ...
    self._times = []   # <<-- collect inference durations
  ...
  def get_control(self, model: mujoco.MjModel, data: mujoco.MjData) -> None:
    ...
    if self._counter % self._n_substeps == 0:
      ...
      # every 100 evals print the average latency
      if len(self._times) % 100 == 0:
        avg_ms = (sum(self._times) / len(self._times)) * 1e3
        print(f"[OnnxController] avg policy eval: {avg_ms:.2f} ms over {len(self._times)} runs")
