...
from mujoco_playground._src.manipulation.doosan_robotics import dsr
...
class PandaPickCube(dsr.PandaBase):
  ...
  def reset(self, rng: jax.Array) -> State:
    ...
    box_pos = (
        jax.random.uniform(
            ...
            maxval=jp.array([0.2, 0.2, 0.0]), # 최대
        )
        + self._init_obj_pos
    )
    ...
    target_pos = (
        jax.random.uniform(
            rng_target,
            (3,),
            minval=jp.array([-0.2, 0.2, 0.0]), # 박스 목표 위치 최소
